...
function GM:PlayerInitialSpawn(ply)
    -- Mark player as initial spawn to handle special cases
    ply.initialspawn = true
    
    -- Set up basic player properties
    ply.viewmode = 3
    
    -- Ensure player has valid appearance from start
    timer.Simple(0.5, function()
        if IsValid(ply) then
            -- Apply random appearance if none set
            if not ply.Appearance then
                local appearance = GetRandomAppearance(ply)
                if appearance and appearance.Model then
                    ply:SetModel(appearance.Model)
                    if appearance.Skin then ply:SetSkin(appearance.Skin) end
                    if appearance.Bodygroups then ply:SetBodyGroups(appearance.Bodygroups) end
                    ply.Appearance = appearance
                end
            end
            
            -- Apply appearance using mode's function if available
...
function GM:PlayerSpawn(ply)
    ...
    -- Late joiner initialization - fix for players joining without appearance/TPIK/HUD
    timer.Simple(0.1, function()
        if IsValid(ply) and ply:Alive() then
            -- Apply appearance if player doesn't have one set
            if not ply.Appearance then
                local appearance = GetRandomAppearance(ply)
                if appearance and appearance.Model then
                    ply:SetModel(appearance.Model)
            ...
            -- Apply appearance using the mode's ApplyAppearance function if available
            local success, error = pcall(function()
                if ApplyAppearance then ApplyAppearance(ply) end
            end)
        end
