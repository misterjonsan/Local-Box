if CLIENT then
    ...
    local function OpenPlayerClassSelection()
        local frame = vgui.Create("ZFrame")
        frame:SetSize(600, 500)
        frame:Center()
        frame:SetTitle("Player Class Manager")
        frame:MakePopup()
        ...
        rightPanel:SetSize(frame:GetWide() / 2 - 10, frame:GetTall())
        rightPanel:Dock(RIGHT)
        rightPanel:DockMargin(5, 5, 5, 5)
        StyleElement(rightPanel, Color(30, 30, 30, 200))

        -- Player List
        local pTitle = vgui.Create("DLabel", leftPanel)
        pTitle:SetText("Select Players")
        pTitle:SetFont("DermaLarge")
        pTitle:SetTextColor(Color(255, 200, 0))
        pTitle:Dock(TOP)
        pTitle:DockMargin(0, 5, 0, 5)
        pTitle:SetContentAlignment(5)

        local pScroll = vgui.Create("DScrollPanel", leftPanel)
        pScroll:Dock(FILL)
        ...
        local function UpdatePlayerList()
            ...
            local allPanel = vgui.Create("DPanel", pScroll)
            allPanel:SetTall(30)
            allPanel:Dock(TOP)
            allPanel:DockMargin(5, 2, 5, 2)
            StyleElement(allPanel, Color(50, 50, 50, 200))

            local allCheck = vgui.Create("DCheckBoxLabel", allPanel)
            allCheck:SetText("SELECT ALL")
            allCheck:Dock(FILL)
            allCheck:DockMargin(10, 5, 0, 0)
            allCheck.OnChange = function(self, val)
                for _, cb in pairs(playerCheckboxes) do
                    if IsValid(cb) then cb:SetChecked(val) end
                end
            end

            for _, ply in ipairs(player.GetAll()) do
                local pnl = vgui.Create("DPanel", pScroll)
                pnl:SetTall(30)
                pnl:Dock(TOP)
                pnl:DockMargin(5, 2, 5, 2)
                StyleElement(pnl, Color(45, 45, 45, 200))

                local cb = vgui.Create("DCheckBoxLabel", pnl)
                cb:SetText(ply:Nick() .. " [" .. (ply:GetPlayerClass() or "none") .. "]")
                cb:Dock(FILL)
                cb:DockMargin(10, 5, 0, 0)
                cb.Player = ply
                playerCheckboxes[ply:UserID()] = cb
                
                if ply == LocalPlayer() then
                    cb:SetTextColor(Color(100, 255, 100))
                end
            end
        end
        ...
        -- Class List
        local cTitle = vgui.Create("DLabel", rightPanel)
        cTitle:SetText("Select Class")
        cTitle:SetFont("DermaLarge")
        cTitle:SetTextColor(Color(255, 200, 0))
        cTitle:Dock(TOP)
        cTitle:DockMargin(0, 5, 0, 5)
        cTitle:SetContentAlignment(5)

        local cScroll = vgui.Create("DScrollPanel", rightPanel)
        cScroll:Dock(FILL)
        cScroll:DockMargin(5, 5, 5, 5)

        local classes = {}
        if player.classList then
        ...
        -- Кнопка сброса класса (опционально, если нужно как в оригинале properties)
        -- table.insert(classes, 1, "none") 

        for _, className in ipairs(classes) do
            local cBtn = vgui.Create("DButton", cScroll)
            cBtn:SetText(className:upper())
            cBtn:SetTall(35)
            cBtn:Dock(TOP)
            cBtn:DockMargin(5, 2, 5, 2)
            StyleElement(cBtn, Color(50, 50, 50, 200))
            ...
            cBtn.DoClick = function()
                ...
                for _, cb in pairs(playerCheckboxes) do
                    if IsValid(cb) and cb:GetChecked() and IsValid(cb.Player) then
                        -- Отправляем сообщение для каждого игрока, точно так же как это делал properties скрипт
                        -- (Entity, затем String)
                        net.Start("AdminSetPlayerClass")
                            net.WriteEntity(cb.Player)
                ...
                if count == 0 then
                    chat.AddText(Color(255, 0, 0), "No players selected!")
                else
                    chat.AddText(Color(0, 255, 0), "Changing class to ", Color(255, 255, 255), className, Color(0, 255, 0), " for " .. count .. " players.")
                    -- Обновляем список, чтобы видеть изменения классов в чекбоксах
                    timer.Simple(0.5, function() 
                        if IsValid(pScroll) then UpdatePlayerList() end 
                    end)
                end
            end
        end
    ...
    local function OpenPlayerClassSelection()
        ...
        for _, className in ipairs(classes) do
            ...
            cBtn.DoClick = function()
                local targets = {}
                for uid, cb in pairs(playerCheckboxes) do
                ...
                chat.AddText(Color(0, 255, 0), "Set class ", Color(255, 255, 255), className, Color(0, 255, 0), " for " .. #targets .. " players.")
                frame:Close()
            end
        end
    end
    ...
    local function OpenAdminMenu()
        ...
        local setClassBtn = vgui.Create("DButton", frame)
        setClassBtn:SetText("Change Player Class")
        setClassBtn:Dock(TOP)
        setClassBtn:DockMargin(5, 2, 5, 2)
        setClassBtn:SetSize(300, 40)
        StyleElement(setClassBtn)
        setClassBtn.DoClick = function()
            OpenPlayerClassSelection()
        end
