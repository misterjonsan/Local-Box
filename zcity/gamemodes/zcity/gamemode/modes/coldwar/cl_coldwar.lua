...
local MODE = MODE
net.Receive("coldwar_start",function()
    surface.PlaySound("lifeless.wav")
...
local teams = {
	[0] = {
		objective = "Kill every orc, or take them hostage. Up to you.",
		name = "an AZOV soldier",
		color1 = Color(90,75,0),
		color2 = Color(90,75,0)
	},
	[1] = {
		objective = "Kill every hohol, don't spare anyone.",
		name = "a RUSICH soldier",
...
local hohColor = Color(150,128,42,205)
local wagColor = Color(33,109,0,205)
local bgColor = Color(0,0,0,155)
local sizeH = ScreenScale(6)
local sizeW = ScreenScale(40)
...
function MODE:HUDPaint()
    ...
    draw.SimpleText("Cold War | Operation Flashpoint", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
net.Receive("coldwar_roundend",function()
    CreateEndMenu()
end)
...
CreateEndMenu = function()
	...
	for i,ply in ipairs(player.GetAll()) do
		...
		but.Paint = function(self,w,h)
			...
			surface.SetFont( "ZB_InterfaceMediumLarge" )
			local lenghtX, lenghtY = surface.GetTextSize( ply:GetPlayerName() or "He left..." )
			
			surface.SetTextColor(0,0,0,255)
			surface.SetTextPos(w / 2 + 1,h/2 - lenghtY/2 + 1)
			surface.DrawText(ply:GetPlayerName() or "He left...")
			...
			surface.DrawText((ply:Name() .. (not ply:Alive() and " - died" or "")) or "He left...")

			surface.SetFont( "ZB_InterfaceMediumLarge" )
			surface.SetTextColor(col.r,col.g,col.b,col.a)
			local lenghtX, lenghtY = surface.GetTextSize( ply:Frags() or "He left..." )
			surface.SetTextPos(w - lenghtX -15,h/2 - lenghtY/2)
			surface.DrawText(ply:Frags() or "He left...")
		end
