...
-- lua/autorun/client/smooth_spectator_noclip.lua

local ACCELERATION = 600
local FRICTION = 2.5
local MAX_SPEED = 1200
local BOOST_MULT = 2.5

local velocity = Vector(0, 0, 0)
local customPos = nil
local isActive = false

hook.Add("CreateMove", "SmoothSpectatorNoclip", function(cmd)
    local ply = LocalPlayer()
    if not IsValid(ply) then return end

    if ply:Alive() or ply:GetObserverMode() ~= OBS_MODE_ROAMING then
        if isActive then
    ...
    if cmd:KeyDown(IN_FORWARD) then wishDir = wishDir + forward end
    if cmd:KeyDown(IN_BACK) then wishDir = wishDir - forward end
    if cmd:KeyDown(IN_MOVERIGHT) then wishDir = wishDir + right end
    if cmd:KeyDown(IN_MOVELEFT) then wishDir = wishDir - right end
    if cmd:KeyDown(IN_JUMP) then wishDir = wishDir + up end
    if cmd:KeyDown(IN_DUCK) then wishDir = wishDir - up end

    local accel = ACCELERATION
    local maxSpd = MAX_SPEED
    if cmd:KeyDown(IN_SPEED) then
        accel = accel * BOOST_MULT
        maxSpd = maxSpd * BOOST_MULT
    end

    if wishDir:LengthSqr() > 0 then
        wishDir:Normalize()
        velocity = velocity + wishDir * accel * dt
    end

    local speed = velocity:Length()
    if speed > maxSpd then
        velocity = velocity * (maxSpd / speed)
        speed = maxSpd
    end
    ...
    customPos = customPos + velocity * dt

    -- Убиваем стандартное движение полностью
    cmd:SetForwardMove(0)
    cmd:SetSideMove(0)
    cmd:SetUpMove(0)
    cmd:SetButtons(bit.band(cmd:GetButtons(), bit.bnot(IN_FORWARD + IN_BACK + IN_MOVELEFT + IN_MOVERIGHT + IN_JUMP + IN_DUCK)))
end)

hook.Add("CalcView", "SmoothSpectatorNoclip", function(ply, pos, angles, fov)
    if not isActive or not customPos then return end
