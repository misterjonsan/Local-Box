...
if gmod then
  ...
  local sendInfoMessage = sendInfoMessage or function(msg, label)
    return print("[" .. tostring(label) .. "] " .. tostring(msg))
  end
  local sendErrorMessage = sendErrorMessage or sendInfoMessage
  Log = function(...)
    return sendInfoMessage(string.format(...), 'Lua Patcher')
  end
  LogError = function(...)
    local message = debug.traceback(string.format(...), 2)
    if not (string.find(message, "'pcall'")) then
      ...
      if times_logged < 10 then
        ...
        times_logged = times_logged + 1
        if times_logged >= 10 then
          sendErrorMessage('Suppressing further logs about the same error.', 'Lua Patcher')
        end
        LUA_PATCHER.tracebacks_logged[message] = times_logged
      end
    end
...
RollbackTable = function(table_name, table_contents)
  if LUA_PATCHER.unpatched[table_name] and table_contents then
...
RollbackFunction = function(func_name)
  ...
  if LUA_PATCHER.unpatched[func_name] then
    LUA_PATCHER.unpatched[func_name] = nil
  end
end
...
UnpatchLibraries = function()
  ...
  return RollbackTable("math", math)
end
local PatchGModLibraries
...
local UnpatchGModLibraries
UnpatchGModLibraries = function()
...
local function sanitize_hook_table(name)
  local ht = hook.GetTable()
  local t = ht and ht[name]
...
OverwriteFunction("hook_Run", function(name, ...)
  sanitize_hook_table(name)
  local f = LUA_PATCHER.unpatched.hook_Run or hook.Run
  return f(name, ...)
end)
timer.Simple(0, function()
  sanitize_hook_table("HomigradDamage")
end)
local PatchClasses
  ...
  local new_entity_metatable = {
    ...
    PhysicsInit = function(self, solidType, ...)
      if solidType == SOLID_NONE then
        ...
        timer.Simple(0, function()
          if (NULL ~= self and IsValid(self:GetPhysicsObject())) then
...
Unpatch = function()
  ...
  if next(LUA_PATCHER.unpatched) then
    Log('...but a leak has happened!! Affected items:')
    for k, v in pairs(LUA_PATCHER.unpatched) do
...
if gmod then
  ...
  hook.Add("Initialize", "lua_patcher", function()
