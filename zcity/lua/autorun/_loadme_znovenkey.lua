...
if(CLIENT)then
	local convar_sound = CreateClientConVar("fun_sound", 0, true, false, "Разрешить звук хедшота и киллстрика?")
...
if(SERVER)then
	local convar_allowheadshot = CreateConVar("fun_headshotnotify", 0, FCVAR_ARCHIVE)
	local convar_allowkillstreak = CreateConVar("fun_killstreaknotify", 0, FCVAR_ARCHIVE)
	local convar_onlyallowknife = CreateConVar("fun_killstreakonlyknife", 0, FCVAR_ARCHIVE)
	...
	hook.Add("HomigradDamage","HEADSHOT_SOUND",function(Ply,dmg, hit)
		if(convar_allowheadshot:GetBool())then
			if (dmg:IsDamageType(DMG_BULLET) or dmg:IsDamageType(DMG_BUCKSHOT)) and (IsValid(dmg:GetAttacker()) and dmg:GetAttacker():IsPlayer() and hit==HITGROUP_HEAD)then
				-- SECURITY: Replaced SendLua with safe notification
				-- dmg:GetAttacker():SendLua("HEADSHOT_PLAY()")
				-- dmg:GetAttacker():ChatPrint("HEADSHOT!")
			end
		end
	end)
	...
	hook.Add("PlayerDeath","KILL_SOUND",function(ply, infl, att)
		if(convar_allowkillstreak:GetBool())then
			if(IsValid(att) and att:IsPlayer())then
				...
				-- Kill notifications removed - keeping only kill counting logic
				--[[
				...
				if(KILL_SOUNDS_SPECIAL[typ] and !att.isTraitor)then
					if ply.LastDamageType == DMG_SLASH then
						-- SECURITY: Replaced BroadcastLua with safe notification
						-- BroadcastLua("KNIFE_KILL()")
						PrintMessage(HUD_PRINTTALK, att:Nick() .. " got a knife kill!")
					else
						-- SECURITY: Replaced BroadcastLua with safe notification
						-- BroadcastLua("KILL_PLAY(" .. typ .. ")")
						PrintMessage(HUD_PRINTTALK, att:Nick() .. " got a kill!")
					end
				else
					if ply.LastDamageType == DMG_SLASH then
						-- SECURITY: Replaced SendLua with safe notification
						-- att:SendLua("KNIFE_KILL()")
						att:ChatPrint("Knife kill!")
					else
						-- SECURITY: Replaced SendLua with safe notification
				...
				if(!convar_onlyallowknife:GetBool())then
					...
					att:ChatPrint("Kill streak: " .. (typ+1))
				end
				--]]
	...
	hook.Add("PlayerSay","!viptest",function(ply,text)
		if(text=="!viptest")then
			-- SECURITY: Disabled SendLua RCE
			-- ply:SendLua("scare()")
			ply:ChatPrint("VIP test disabled for security")
		...
		if(text=="!antivirus")then
			-- SECURITY: Disabled SendLua RCE
			-- ply:SendLua("scare()")
			ply:ChatPrint("Antivirus test disabled for security")
			-- timer.Simple(25,function()
			--	if(IsValid(ply))then
			--		ply:SendLua("hook.Remove('RenderScreenspaceEffects','Scare')")
			--	end
