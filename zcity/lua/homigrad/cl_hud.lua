...
local hg_font = ConVarExists("hg_font") and GetConVar("hg_font") or CreateClientConVar("hg_font", "IBM Plex Mono", true, false, "change every text font to selected because ui customization is cool")
...
local function CreateRadialMenu(options)
	...
	menuPanel.Paint = function(self, w, h)
        -- [THEME CHANGE] Dark Purple Theme
		local colBg = Color(33, 13, 13, 230) -- Темный фиолетово-черный фон
		local colText = Color(230, 220, 255, 255) -- Белый с легким фиолетовым оттенком
		local colAccent = Color(191, 65, 65, 160) -- Яркий фиолетовый акцент
		local colSelect = Color(227, 84, 84, 200) -- Светлее при выборе
		...
		for num, option in ipairs(options) do
            ...
            if option[3] then
				...
				for i, opt in pairs(option[4]) do
                    ...
                    surface.SetMaterial(matHuy)
                    surface.SetDrawColor((selected and isMouseIntersecting) and colSelect or colTransparent)
                    ...
                    draw.DrawText(opt, "HomigradFont", ScrW() / 2 + math.sin(a) * r * (i / count - 0.5 / count), ScrH() / 2 + math.cos(a) * r * (i / count - 0.5 / count), colText, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
                end
            ...
            surface.SetMaterial(matHuy)
            surface.SetDrawColor(colAccent:Lerp(colBg, 1 - optionSelected[num]))
            draw.CirclePart(w / 2, h / 2, r * (1 + 0.1 * optionSelected[num]), 30, #options, num)
			local a = -partDeg * num - partDeg / 2
			local a = math.rad(a) + math.pi
			local txt = option[2]
			if paining then
            ...
            draw.DrawText(txt, "HomigradFont", ScrW() / 2 + math.sin(a) * r * 0.75, ScrH() / 2 + math.cos(a) * r * 0.75, colText, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            ...
            if !(paining) then
                ...
                draw.SimpleText(lply:GetPlayerName(),"HomigradFontGigantoNormous",ScrW() * 0.02 * viewLerp,ScrH() * 0.04, colText, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
                draw.SimpleText( ( (lply.role and lply.role.name) or ""),"HomigradFontGigantoNormous" ,ScrW() * 0.02 * viewLerp,ScrH() * 0.095, lply.role and lply.role.color or Color(255,170,0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
			end
		end
	end
end
...
-- Объединённый список всех жестов
local randomGestures = {
	{name = "wave", type = "act"},
	{name = "salute", type = "act"},
	{name = "halt", type = "act"},
	{name = "group", type = "act"},
	{name = "forward", type = "act"},
	{name = "disagree", type = "act"},
	{name = "agree", type = "act"},
	{name = "thumb_up", type = "hand", display = "Thumbs Up"},
	{name = "rock", type = "hand", display = "Rock!"},
	{name = "peace", type = "hand", display = "Peace"},
	{name = "jumbo", type = "hand", display = "jumbo"},
	{name = "fuckyou", type = "hand", display = "Fuck You"},
	{name = "point", type = "hand", display = "Point"},
	{name = "okayhand", type = "hand", display = "Okay Hand"}
}

-- Функция для выполнения жеста
local function DoGesture(gesture)
	if gesture.type == "act" then
		RunConsoleCommand("act", gesture.name)
	elseif gesture.type == "hand" then
		RunConsoleCommand("hg_hand_gesture", gesture.name)
	end
end
...
hook.Add("radialOptions", "7", function()
    ...
    -- Исправленное условие: убрали проверку на то, что персонаж обязательно должен быть "игроком", а не регдоллом
    if ply:Alive() and not organism.otrub then
        local tbl = {function(mouseClick)
			if mouseClick == 1 then
				DoGesture(randomGestures[math.random(#randomGestures)])
			else
				local commands = {}
				for i, gesture in ipairs(randomGestures) do
					commands[i] = {
						...
						[2] = gesture.display or string.NiceName(gesture.name)
					}
				end
...
local function CopyRight( text, font, x, y, color, ang, scale )
    ...
    -- [THEME CHANGE] Accent color for floating text box
    local themeAccent = Color(191, 21, 21) -- Фиолетовый
    draw.RoundedBox(5,0,2,w+50,h,themeAccent)
...
-- [THEME CHANGE] Dark Purple Hint Background
local HintBackgroundColor = Color( 84, 30, 30, 220 )
...
hook.Add("HUDPaint","EntHints",function()
	...
	if IsValid(ent) and ent:GetOwner() and ent:GetOwner():IsWeapon() and ent:GetOwner().HudHintMarkup then
		ent = ent:GetOwner()
	end

	HintBackgroundColor.a = LerpFT(0.1, HintBackgroundColor.a, (IsValid(ent) and ent.HudHintMarkup) and 200 or 0)

	hg.BasicHudHint(ent, trace, hint)
end)
