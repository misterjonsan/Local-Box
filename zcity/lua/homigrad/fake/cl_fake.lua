...
hook.Add("InputMouseApply", "fakeCameraAngles", function(cmd, x, y, angle)
	...
	if hg_coolcamera:GetBool() then
		realangle = angle
		realanglelerp = LerpAngleFT(0.03 * (hg_coolcamera:GetFloat() or 1), realanglelerp, realangle)
		cmd:SetViewAngles(realanglelerp + GetViewPunchAngles2() * 1 + GetViewPunchAngles() * 1 + GetViewPunchAngles3() * 1 + GetViewPunchAngles4() * 1)
	else
...
-- Загружаем 4 иконки (2 открытые, 2 закрытые/хватающие)
local matLeftHand = Material("materials/left-hand.png", "noclamp smooth")
local matRightHand = Material("materials/right-hand.png", "noclamp smooth")
local matLeftHandGrab = Material("materials/left-hand2.png", "noclamp smooth")
local matRightHandGrab = Material("materials/right-hand2.png", "noclamp smooth")

local hg_fake_icons = ConVarExists("hg_fake_icons") and GetConVar("hg_fake_icons") or CreateConVar("hg_fake_icons", 0, FCVAR_ARCHIVE, "Homigrad Rework like icons", 0, 1)

-- Функция для проверки: касается ли рука чего-либо
-- Мы создаем маленький невидимый кубик (5x5x5 юнитов) вокруг руки и проверяем, задевает ли он мир
local function IsTouchingSomething(pos, ent)
	local tr = util.TraceHull({
		start = pos,
		endpos = pos,
		mins = Vector(-5, -5, -5), -- Размер куба проверки
		maxs = Vector(5, 5, 5),
		filter = ent, -- Игнорируем самого игрока/рэгдолл
		mask = MASK_SOLID -- Проверяем только твердые объекты
	})
	return tr.Hit
end

hook.Add("HUDPaint", "HG_DrawHandIcons", function()
	-- Если выключено в настройках - не рисуем
	if not hg_fake_icons:GetBool() then return end
	...
	-- Кнопки, отвечающие за руки (Shift - левая, Alt - правая)
	local showLeft = input.IsKeyDown(KEY_LSHIFT)
	local showRight = input.IsKeyDown(KEY_LALT)
	...
	if showLeft then
		...
		if bone then
			...
			-- Проверяем, касается ли рука чего-то
			local isGrabbing = IsTouchingSomething(pos, ent)
			local matToUse = isGrabbing and matLeftHandGrab or matLeftHand

			local scr = pos:ToScreen()
			if scr.visible then
				surface.SetMaterial(matToUse)
				surface.SetDrawColor(255, 255, 255, 255)
				surface.DrawTexturedRect(scr.x - 85, scr.y - 65, 170, 130)
			end
		end
	end
	...
	if showRight then
		...
		if bone then
			...
			-- Проверяем, касается ли рука чего-то
			local isGrabbing = IsTouchingSomething(pos, ent)
			local matToUse = isGrabbing and matRightHandGrab or matRightHand
