...
if SERVER then
    util.AddNetworkString("PlayFurryTransSound")
end

if CLIENT then
    net.Receive("PlayFurryTransSound", function()
        local ply = net.ReadEntity()
        if not IsValid(ply) then return end

        sound.PlayURL("https://www.myinstants.com/media/sounds/changed-transfur-sound.mp3", "3d", function(station)
            if IsValid(station) then
                station:SetPos(ply:GetPos())
                station:SetVolume(1)
                station:Set3DFadeDistance(300, 1500)
                station:Play()
                
                -- Хук для привязки звука к игроку
                local hookName = "FurryTransSound_" .. ply:EntIndex() .. "_" .. CurTime()
                hook.Add("Think", hookName, function()
                    if IsValid(station) and IsValid(ply) and station:GetState() == GMOD_CHANNEL_PLAYING then
                        station:SetPos(ply:GetPos())
                    else
...
function CLASS.On(self, data)
	...
	if data.instant then
		...
		self:SetModel("models/akittu/stylizedexpie/akittuexpie.mdl")
	...
	if CLIENT then
		...
		self.mdlfur = ClientsideModel("models/akittu/stylizedexpie/akittuexpie.mdl")
...
local bluewhite = Color(205, 255, 187)
...
function CLASS.HUDPaint(self)
	...
	render.SetScissorRect(0, sh * 0.75 - (assimilation * ScreenScale(20)), sw, sh, true)
		surface.SetFont("ZB_ProotOSAssimilation")
		surface.SetTextColor(193, 255, 164, alpha)
		local txt = "Assimilating..."
		local w, h = surface.GetTextSize(txt)
		surface.SetTextPos(sw * 0.5 - w * 0.5, sh * 0.75 - h - ScreenScale(5))
		surface.DrawText(txt)
...
if CLIENT then
	local scancolor = Color(124, 220, 60)
